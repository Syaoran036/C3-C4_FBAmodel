...
#       
      # constrain PEPC flux
      lowbnd(C4FBAmodel.const)[which(react_id(C4FBAmodel.const)=="R00345_c")] <- -vCres$Vp
      # force decarboxylation in BS by NADPME
      #uppbnd(C4FBAmodel.const)[which(react_id(C4FBAmodel.const)=="R00216_pb")] <- vCres$Vp
...
#       #constrain Glycine and Serine plasmodesmata exchange 
      ...
      uppbnd(C4FBAmodel.const)[which(react_id(C4FBAmodel.const)=="R01221_mb")] <- 0.5 * vCres$xi * vCres$Vom
      uppbnd(C4FBAmodel.const)[which(react_id(C4FBAmodel.const)=="R01221_m")] <- 0.5 * (1-vCres$xi) * vCres$Vom

      #GOGAT constrain
      uppbnd(C4FBAmodel.const)[which(react_id(C4FBAmodel.const)=="R00021_p")] <- vCres$GluS
      lowbnd(C4FBAmodel.const)[which(react_id(C4FBAmodel.const)=="R00021_p")] <- vCres$GluS
      uppbnd(C4FBAmodel.const)[which(react_id(C4FBAmodel.const)=="R00021_pb")] <- vCres$GluS
      lowbnd(C4FBAmodel.const)[which(react_id(C4FBAmodel.const)=="R00021_pb")] <- vCres$GluS
