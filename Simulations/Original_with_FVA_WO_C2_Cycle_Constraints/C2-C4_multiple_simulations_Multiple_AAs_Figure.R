...
#FBAmodFile="C:/Users/David/Documents/Bio/PhD/Models/Gomes de Oliveira Model/vC_FBA_1_2013/C2toC4/C4GEM_maize model_H2Oplasmo_C2_Biomass_joined.xml"
...
Vpmax.range<-seq(0,10,length=36)
...
Ala_AT_cytop<-unlist(lapply(res$C4fluxes, function(x){as.numeric(x["Ala-AT_cytop","vCFBAres.flux"])}))
Ala_AT_cytop_bs<-unlist(lapply(res$C4fluxes, function(x){as.numeric(x["Ala-AT_cytop_bs","vCFBAres.flux"])}))


aKG_MtoBS<-unlist(lapply(res$plasmoFluxes, function(x){as.numeric(x["aKG MtoBS","flux"])}))
...
par(mar=c(5.1, 4.8, 2.1, 2.1),
    cex=0.6)

#"Black=AlaAT_m, Grey=Ala_AT_bs, Black_dashed=aKG_MtoB, grey_dashed=glu_MtoBS",
...
plot(Vpmax.range,
     ...
     ylab=expression("flux [ µmol"%*%"m"^-2%*%"s"^-1*"]"),     
     #main="Black=AlaAT_m, Grey=Ala_AT_bs, blue=aKG_MtoB, green=glu_MtoBS",
     main=NULL,
     type="l"
)
# ala AT is defined ala-> pyr
lines(Vpmax.range, -Ala_AT_cytop_bs, col="grey")
lines(Vpmax.range, aKG_MtoBS, col="black",lty=2)
lines(Vpmax.range, -glu_MtoBS, col="grey",lty=2)
#lines(Vpmax.range,-ala_MtoBS,col="red")

#windows()
#plot(Vpmax.range,ala_MtoBS, ylim= extendrange(c(glu_MtoBS,ala_MtoBS)),main="Black=Ala_over_pmd, Grey=Glu_over_pmd")
#points(Vpmax.range,glu_MtoBS,col="grey")
...
# ala AT is defined ala-> pyr
points(Vpmax.range, -Ala_AT_cytop_bs, col="grey",ps=0.5)
points(Vpmax.range, aKG_MtoBS, col="black",pch=2,ps=0.5)
points(Vpmax.range, -glu_MtoBS, col="grey",pch=2,ps=0.5)
