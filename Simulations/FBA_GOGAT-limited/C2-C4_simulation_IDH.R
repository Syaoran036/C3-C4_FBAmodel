...
source(file="Models/C2-C4_IDH-limited.R")
...
IDH.range<-seq(0,1,length=31)

res<-list(sol.stat=rep(NA,length=length(IDH.range)),C4fluxes=vector(mode="list",length=length(IDH.range)), 
          plasmoFluxes= vector(mode="list",length=length(IDH.range) ),
          vCres=vector(mode="list",length=length(IDH.range) ),
          obj=rep(NA,length=length(IDH.range))
)
...
for (i in seq_along(IDH.range)){
  
  parms<-c(beta=0.65, kcat=2.77, Vpmax=6, Kp=80, gs=15*1e-3, xi=0.58,IDH=IDH.range[i]) #Specific for F.ram Type II C3-C4
  ...
  vCFBAres<-vC.C3C4.FBA(pars=parms,
                        ...
                        Om=O,
                        Etot=Etot,
                        mechModFile="Models/vC C3C4 Cm Landscape model Rubisco pars kcat dependent FAST_IDH.r",
...
pdf("PLOT/plot_IDH.pdf",width=8.5*0.3937,height=6.5*0.3937)
...
GDC_mito_BS<-unlist(lapply(res$C4fluxes, function(x){as.numeric(x["GDC mito BS","vCFBAres.flux"])}))
...
gly_MtoBS<-unlist(lapply(res$plasmoFluxes, function(x){as.numeric(x["gly MtoBS","flux"])/2}))
Malate_MtoBs<-unlist(lapply(res$plasmoFluxes, function(x){as.numeric(x["Malate MtoBs","flux"])}))

plot(IDH.range,
     Malate_MtoBs, 
     xlim=c(min(IDH.range),max(IDH.range)),
     ylim=c(min(min(Malate_MtoBs),min(gly_MtoBS)),max(max(Malate_MtoBs),max(gly_MtoBS))),
     xlab=expression("IDH activity [µmol"%*%"m"^-2%*%"s"^-1*"]"),
     ylab=expression("Plasmo. fluxes [µmol"%*%"m"^-2%*%"s"^-1*"]"),     
     #main="Black=AlaAT_m, Grey=Ala_AT_bs, blue=aKG_MtoB, green=glu_MtoBS",
     main=NULL,
     type="l"
)
lines(IDH.range, gly_MtoBS, col="grey")
...
legend("topleft",legend=c("Malate MtoBs","1/2 Gly MtoBS"),ncol=1,cex=0.8,bty="n",col=c("black","grey"),lty=c(1,1))
dev.off()
